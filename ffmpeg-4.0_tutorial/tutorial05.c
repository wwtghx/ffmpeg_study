...
// Use
//
//gcc -w -o a.out tutorial05.c -lavutil -lavformat -lavcodec -lswscale -lz -lm -lSDL -lpthread
//
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size, double *pts_ptr) {
    ...
    for(;;) {
        while(is->audio_pkt_size > 0) {
...
#endif
            if( got_frame ) {
                int in_samples = is->audio_frame.nb_samples;
                short *sample_buffer = (short*)malloc(is->audio_frame.nb_samples * 2 * 2);
                memset(sample_buffer, 0, is->audio_frame.nb_samples * 4);
                
                int i=0;
                float *inputChannel0 = (float*)(is->audio_frame.extended_data[0]);
                
                // Mono
                if( is->audio_frame.channels == 1 ) {
                    ...
                    float* inputChannel1 = (float*)(is->audio_frame.extended_data[1]);
                    for( i=0; i<in_samples; i++) {
            ...
            if (is->audio_frame.nb_samples <= 0)
            {
                continue;
            }
            
            data_size = is->audio_frame.nb_samples * 4;
